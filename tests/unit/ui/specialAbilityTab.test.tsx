...
describe("AbilityPicker（UI-T3-01〜04・23）", () => {
  it("UI-T3-01: 表示名で検索できる", () => {
    ...
    expect(screen.getByTestId("ability-add-strikeout")).toBeTruthy();
    expect(screen.queryByTestId("ability-add-power_hitter")).toBeNull();
  });
  ...
  it("UI-T3-03（対比）: 野手プランでは投手専用能力を表示しない", () => {
    ...
    expect(screen.queryByTestId("ability-add-strikeout")).toBeNull();
  });
  ...
  it("UI-T3-04: 種別フィルタ「金特」で金特のみになる", () => {
    ...
    expect(screen.getByTestId("ability-add-archartist")).toBeTruthy();
    expect(screen.queryByTestId("ability-add-strikeout")).toBeNull();
  });
...
describe("選択中の青特（UI-T3-05〜11・24）", () => {
  it("UI-T3-05: 追加すると一覧に行が増え、ボタンが「追加済み」になる", () => {
    render(<SpecialAbilityTab />);
    fireEvent.click(screen.getByTestId("ability-add-strikeout"));

    expect(screen.getByTestId("blue-row-strikeout")).toBeTruthy();
    const button = screen.getByTestId("ability-add-strikeout") as HTMLButtonElement;
  ...
  it("UI-T3-07 / UI-T3-08: rank 青特は G〜A を選べ、D→A は3遷移分を合計する", () => {
    setPlan({ playerType: "fielder" });
    render(<SpecialAbilityTab />);
    fireEvent.click(screen.getByTestId("ability-add-chance"));
  ...
  it("UI-T3-24: 投手プランで野手専用青特を追加しても実測どおりに計上される", () => {
    render(<SpecialAbilityTab />);
    fireEvent.click(screen.getByTestId("ability-add-power_hitter"));
    recalculate();
...
describe("選択中の金特（UI-T3-12〜22）", () => {
  it("UI-T3-12 / UI-T3-13: 下位青特が🔗付きで自動追加され、削除ボタンを持たない", () => {
    setPlan({ playerType: "fielder" });
    render(<SpecialAbilityTab />);
    fireEvent.click(screen.getByTestId("ability-add-archartist"));

    expect(screen.getByTestId("blue-row-power_hitter")).toBeTruthy();
    expect(textOf("blue-auto-power_hitter")).toContain("🔗");
    expect(textOf("blue-auto-power_hitter")).toContain("アーチストの前提として自動追加");
  ...
  it("UI-T3-15 / UI-T3-16: 手動追加と自動追加が重なっても1行・1回だけ計上する", () => {
    ...
    expect(textOf("blue-auto-power_hitter")).toContain("前提でもあります");
  ...
  it("UI-T3-22: rank 前提の金特は目標ランク A で自動追加される", () => {
    setPlan({ playerType: "fielder" });
    render(<SpecialAbilityTab />);
    fireEvent.click(screen.getByTestId("ability-add-clutch_master"));
  ...
  it("前提を持たない金特では下位青特のコツLvを表示しない", () => {
    setPlan({ playerType: "fielder" });
    render(<SpecialAbilityTab />);
    fireEvent.click(screen.getByTestId("ability-add-laser_beam"));
